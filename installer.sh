...
cleanup() {
    ...
    [ -d "/tmp/backsncovers-main" ] && rm -rf "/tmp/backsncovers-main"
}
...
if [ $? -ne 0 ]; then
    echo "Failed to extract backsNcovers package!"
    cleanup
    exit 1
fi
...
if [ -d "$TMPPATH/backsncovers-main/usr/lib/enigma2/python/Plugins/Extensions/backsNcovers" ]; then
    cp -r "$TMPPATH/backsncovers-main/usr/lib/enigma2/python/Plugins/Extensions/backsNcovers"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from standard plugin directory"
elif [ -d "$TMPPATH/backsncovers-main/usr/lib64/enigma2/python/Plugins/Extensions/backsNcovers" ]; then
    cp -r "$TMPPATH/backsncovers-main/usr/lib64/enigma2/python/Plugins/Extensions/backsNcovers"/* "$PLUGINPATH/" 2>/dev/null
    echo "Copied from lib64 plugin directory"
elif [ -d "$TMPPATH/backsncovers-main/usr" ]; then
    # Copy entire usr tree
    cp -r "$TMPPATH/backsncovers-main/usr"/* /usr/ 2>/dev/null
